...
contract MarmosetProject is Ownable {
    ...
    uint constant ONE_WEEK = 7200;

    constructor(address _admin) {
        admin = _admin;
        nextLockTime.push(1666958400); //lock
        nextLockTime.push(1666958700); //1 number
        nextLockTime.push(1666959000); //2 number
        nextLockTime.push(1666959300); //3 number
        nextLockTime.push(1666959600); //4 number
        nextLockTime.push(1666959900); //5 number
        nextLockTime.push(1666960200); //6 number
        nextLockTime.push(1666960500); //unlock
    }

    //for test each 2 hours
    ...
    uint[] private nextLockTime; //timestamp of [0 - lock_buy, 1 number .... 6 number, 7 - unlock_buy]
    ...
    function buyToy(uint amount) external {
        ...
        IERC20(mmtToken).transferFrom(msg.sender, address(this), (toPay * amount));
